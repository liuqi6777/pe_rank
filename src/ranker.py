...
class PointwiseTextRanker(Ranker):
    ...
    def __call__(self, query: str, doc: str, **kwargs):
        input_ids = self._get_llm_inputs(query, doc)
        extra_text_input_ids = self._get_encoder_inputs([doc])
        ...
        return logits[0, -1, self._tokenizer.convert_tokens_to_ids("yes")].item()

    def _get_message(self, query: str, doc: str) -> str:
        instruction = """Given a query, does the passage answer the query? Only output \"yes\" or \"no\".
